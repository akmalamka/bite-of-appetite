  ...
  >
    ...
    >
      <PaginationPrev class="mr-4 size-10 flex items-center justify-center rounded-full hover:bg-black/5 disabled:opacity-50 focus-within:outline-1 focus-within:outline-offset-1 focus-within:outline">
        <i
          class="i-material-symbols:chevron-left-rounded"
        />
      </PaginationPrev>
      <template v-for="(page, index) in items">
        <PaginationListItem
          v-if="page.type === 'page'"
          :key="index"
          class="data-[selected]:bg-primary data-[selected]:color-primary-light data-[selected]:hover:bg-primary/90 size-10 border rounded-full transition hover:bg-black/5 focus-within:outline-1 focus-within:outline-offset-1 focus-within:outline"
          :value="page.value"
        >
          {{ page.value }}
        </PaginationListItem>
      ...
      <PaginationNext class="ml-4 size-10 flex items-center justify-center rounded-full hover:bg-black/5 disabled:opacity-50 focus-within:outline-1 focus-within:outline-offset-1 focus-within:outline">
        <i
          class="i-material-symbols:chevron-right-rounded"
        />
      </PaginationNext>
    </PaginationList>
  </PaginationRoot>
